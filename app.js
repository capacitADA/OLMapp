...
import { loginFirebase, logoutFirebase, onAuthChange, fsGetAll, fsAdd, fsUpdate, fsDelete, fsListen, driveSignIn, driveIsConnected, driveUploadHTML } from "./firebase.js";
...
async function doLogin() {
    ...
    // Firebase Auth con email derivado de cédula
    const email = tec.email || (cedula + '@olm.app');
    try {
        await loginFirebase(email, clave + cedula.slice(-4));
    } catch(authErr) {
        // Si no existe en Auth, continuar solo con datos locales
        console.warn('Firebase Auth:', authErr.code);
    }
    sesionActual = tec;
    actualizarTopbar();
    closeModal();
